      ...
      <Row className='mt-2 container'>
       ...
       Wishlist?.length>0?Wishlist.map(product=>(
        ...
        <Card style={{ width: '18rem', height:'' }}>
         ...
         <Card.Img variant="top" style={{width:"100%",minHeight:"250px"}} src={product.thumbnail} />
         </Link>
          <Card.Body>
            <Card.Title style={{minHeight:"60px"}}>{product.title}</Card.Title>
            <div className="d-flex justify-content-between">
              <Button style={{backgroundColor:"red"}}  onClick={() => dispatch(removeFromWishlist(product.id))}>
              🗑️
              </Button>
              <Button style={{backgroundColor:"orange"}} onClick={()=>handleAddToCart(product)} >
               🛒
              </Button>
            </div>
